var Game = function () {
  ...
  this.config = {
    ...
    jumperHeight: 1.5, // jumper高度
    jumperDeep: 1, // jumper深度

    oneStepScore: 10, //每一次成功的分数
  }
  // 游戏状态
  this.score = 0
  this.size = {
  ...
  _handleMouseup: function () {
    ...
    function landed() {
      ...
      if (self.falledStat.location === 1) {
        ...
        self.score += self.config.oneStepScore
        self._createCube()
        self._updateCamera()
  ...
  **/
  ...
  _createCube: function () {
    ...
    if( this.cubes.length ) {
      ...
      if (this.cubeStat.nextDir === 'left') {
        mesh.position.x = this.cubes[this.cubes.length - 1].position.x-4*Math.random() - 3
      } else {
        mesh.position.z = this.cubes[this.cubes.length - 1].position.z-4*Math.random() - 3
      }
    }
  ...
  _setLight: function () {
    var light = new THREE.AmbientLight( 0xa5b5c3, 1 )
